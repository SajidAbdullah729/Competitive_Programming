...
void Build(ll node,ll left,ll right)
{
    ...
    tree[node]=fun(tree[templeft],tree[tempright]);
}

ll Query(ll node,ll left,ll right,ll i,ll j)
{
     if(i>right | j<left) return GG;
...
void S()
{
    ...
    while(q--)
    {
        ...
        else
        {
            ...
            ll g1=fun(lagbe)-fun(lagbe-ss);
            ll kp=tt+1;
            if(kp>=n) { cout<<tt+1<<" ";continue; }
            ll val2=Query(1,0,n-1,pos,kp);
            //deb(val2);

            ll g2=fun(lagbe)-fun(lagbe-val2);
            //deb2(g2,g1);
            if(g2>g1)
            {
                cout<<kp+1<<" ";
