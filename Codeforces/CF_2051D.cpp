...
void S()
{
    ll n,x,y;cin>>n>>x>>y; vll v(n);
    ll cc=0;
    rep(i,0,n) {cin>>v[i];cc+=v[i];}
    //vll v2=v;
    //multiset<ll> st;
    multi_ordered_set<ll> st;
    rep(i,0,n)
    {
       st.insert(v[i]);
    }
    ...
    rep(i,0,n-1)
    {
        ...
        //deb2(aa,bb);
        //deb(st.size());
        //cout<<"h --> ";for(auto xx:st) cout<<xx<<" ";cout<<endl;
        ll g1=st.order_of_key(aa+1);
        ll g2=st.order_of_key(bb);
        //deb2(g1,g2);
        if(g1>g2)
        {
            ans+=(g1-g2);
