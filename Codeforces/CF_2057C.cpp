...
tuple<ll,ll,ll> fun2(tuple<ll,ll,ll> &a,tuple<ll,ll,ll> &b)
{
    if(get(a)>=get(b))
    {
        //deb(get(a));
...
string fun(ll val)
{
    return lltobinary(val);
}
void get_val(ll x,ll y,ll z)
{
    /*deb(fun(x));
    deb(fun(y));
    deb(fun(z));
...
void S()
{
   ...
   string s1=lltobinary(left);
   string s2=lltobinary(right);
   ll len=s1.length();
   ll pos=-1;
   for(ll i=0;i<len;i++)
   {
   ...
   vll v1;
   //deb3(pos,ans1,ans2);
   v1.pb(binaryToDecimal(ans1));
   v1.pb(binaryToDecimal(ans2));
