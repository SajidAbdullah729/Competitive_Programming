...
/*
    find the index of the element using `order_of_key`
    store it in `idx`
    delete using `s.erase(s.find_by_order(idx))`

*/
const ll RR=998244353;
...
ll SUB(ll x,ll y)
{
    return ((x%RR)-(y%RR)+RR)%RR;
}

ll DIV(ll x,ll y)
{
    return ((x%RR)*(modInverse(y,RR)%RR))%RR;
}
void S()
{
    ll n,q;cin>>n>>q;
    ...
    while(q--)
    {
        ...
        if(op==1)
        {
            ...
            ll lastpos=*it;
            ll mini=min(cur,v2[lastpos]);
            ll pore=min(cur+1,v2[lastpos]);
            ans=DIV(ans,mini);
            ans=MUL(ans,pore);

            v1[lastpos]++;
            m1[cur].erase(*it);
            m1[cur+1].insert(lastpos);
            p1[pos]++;
        ...
        else
        {
            ll cur=p2[pos];
            ll hobe=p2[pos]+1;
            auto it=m2[cur].end();it--;
            ll lastpos=*it;
            ll mini=min(cur,v1[lastpos]);
            ll pore=min(cur+1,v1[lastpos]);
            ans=DIV(ans,mini);
            ans=MUL(ans,pore);

            v2[lastpos]++;
            m2[cur].erase(it);
            m2[cur+1].insert(lastpos);
            p2[pos]++;
