...
void S()
{
    ...
    ll n;cin>>n;vll aa(n),bb(n);
    multiset<ll> alice,bob;
    //rep(i,0,n)cin>>v[i];
    rep(i,0,n)cin>>aa[i];
    rep(i,0,n)cin>>bb[i];
    map<ll,vll> mm1,mm2;
    ...
    for(ll i=0;i<n;i++)
    {
        if((i&1))
        {
            ...
            ll temp=*it;
            ll pos=mm2[temp].back();
            mm2[temp].pop_back();
            sol-=(bb[pos]-1);
            ll val2=aa[pos]-1+bb[pos];
            alice.erase(alice.find(val2));
            mm1[val2].pop_back();
            bob.erase(it);
        }
        else
        {
             ...
             ll pos= mm1[temp].back();
             mm1[temp].pop_back();
             sol+=(aa[pos]-1);
             ll val2=bb[pos]-1+aa[pos];
             bob.erase(bob.find(val2));
             mm2[val2].pop_back();
             alice.erase(it);
        }
    }
