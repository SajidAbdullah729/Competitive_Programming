...
ll avg;
ll pref[110];
ll fun(ll idx,ll aa,ll bb)
{

    if(dp[idx][aa][bb]!=-1) return dp[idx][aa][bb];
    if(idx>=n)
    {
        if(aa==avg && bb==avg) return 0;
        else return 1e18;
    }
    ll xx=1e18,yy=1e18,zz=1e18;
    if(v[idx].F==1)
    {

        if(aa+v[idx].S<=avg) xx=fun(idx+1,aa+v[idx].S,bb);
        if(bb+v[idx].S<=avg) yy=fun(idx+1,aa,bb+v[idx].S)+1;
        ll tt=pref[idx]-(aa+bb);
        if(tt+v[idx].S<=avg) zz=fun(idx+1,aa,bb)+1;
    }
    else if(v[idx].F==2)
    {

        if(aa+v[idx].S<=avg) xx=fun(idx+1,aa+v[idx].S,bb)+1;
        if(bb+v[idx].S<=avg) yy=fun(idx+1,aa,bb+v[idx].S);
        ll tt=pref[idx]-(aa+bb);
        if(tt+v[idx].S<=avg) zz=fun(idx+1,aa,bb)+1;
    }
    else if(v[idx].F==3)
    {
        ...
        if(bb+v[idx].S<=avg) yy=fun(idx+1,aa,bb+v[idx].S)+1;
        ll tt=pref[idx]-(aa+bb);
        if(tt+v[idx].S<=avg) zz=fun(idx+1,aa,bb);
    }

    return dp[idx][aa][bb]=min({xx,yy,zz});
