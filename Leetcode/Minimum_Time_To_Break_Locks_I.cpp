...
ll fun(ll n,ll idx,ll mask,vector<vll> &dp,vector<vector<bool>> &vis,vll &v,vll &strength)
{
    if(idx>=n) return 0;
    ...
    for(ll i=0;i<n;i++)
        {
            ...
            ll nmask=MakeOneBit(mask,i);
            //minn=min(minn,0);
            ll pp=(fun(n,idx+1,nmask,dp,vis,v,strength)+Ceil((strength[idx]*1.00)/v[i]));
            minn=min(minn,pp);
        }
    vis[idx][mask]=1;
    return dp[idx][mask]=minn;
}
...
public:
    int findMinimumTime(vector<int>& strength, int K) {
        ...
        ll n=strength.size();
        ll pp=(1<<(n+2));
        vector<vll> dp(n+2,vll(pp,-1));
        vector<vector<bool>> vis(n+2,vector<bool>(pp,0));
        vector<ll> tt;
        ll k=1;
        v.pb(k);
        ...
        return fun(n,0,0,dp,vis,v,strength);
    }
};