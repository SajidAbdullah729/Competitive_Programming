...
pll bfs(ll source,v2d &adj, vll &vis,vll &level,ll n)
{
   queue<ll> q;
    q.push(source);
    level[source]=0;
...
public:
    vector<int> maxTargetNodes(vector<vector<int>>& edges1, vector<vector<int>>& edges2) {
         ll n=edges1.size();
         ll m=edges2.size();
         vector<vector<ll>>adj(n+5),adj2(m+5);
         for(ll i=0;i<n;i++)
             {
                ll xx=edges1[i][0];
                ll yy=edges1[i][1];
        ...
        ll ans=0,maxi=1e9;
        vector<ll> dist(n+5,maxi),vis(n+5,0);
        vector<ll> dist2(m+5,maxi),vis2(m+5,0);
        
         clearg(vis,0);clearg(dist,maxi);
         pll tt=bfs(0,adj,vis,dist,n);
         
                
        //deb(ans);
        vll sol;
        
        clearg(vis2,0);clearg(dist2,maxi);
        pll tt2=bfs(0,adj2,vis2,dist2,m);
        ll g1=max(tt2.F,tt2.S);
        for(ll i=0;i<=n;i++)
            {
                ll pq=0;
                if(dist[i]%2==1)
