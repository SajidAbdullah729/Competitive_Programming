...
Tppp void debugvpll(vector<pair<T1,T2> > v) { for(ll i=0;i<v.size();i++){deb(i);deb(v[i].F);deb(v[i].S);cout<<endl;}}
void debSpec(vll v,ll pos){ cout<<pos<<"="<<v[pos]<<endl; }
Tppp void debugmap(map<T1,T2> m) {ll i=0;for(auto x:m) {deb(i);deb(x.F);deb(x.S);cout<<endl;i++;}}
Tpp void debugset(set<T> s) {for(auto x:s) {deb(x);cout<<endl;}}
Tpp void debugmultiset(multiset<T> s) {for(auto x:s) {deb(x);cout<<endl;}}

Tpp void debugpqueue(priority_queue<T> pq) {while(!pq.empty()){deb(pq.top());pq.pop();}    }
Tpp void debugstack(stack<T> st) {while(!st.empty()) {deb(st.top());st.pop();}}
Tpp void debugqueue(queue<T> q) {while(!q.empty()){ deb(q.front());q.pop(); }}

void debuggraph(vector<ll> adj[],ll node){rep(i,0,node+1){deb(i);rep(j,0,adj[i].size()) {cout<<adj[i][j]<<" ";}cout<<endl;}cout<<endl;}
...
Tpp void Inputvec(vector<T> &v,ll n){rep(i,0,n){T d;cin>>d;v.pb(d);}}
Tppp void Inputvpll(vector<pair<T1,T2> > &v,ll n){rep(i,0,n){T1 a;T2 b;cin>>a>>b;v.pb(mp(a,b));} }
void Inputgraph(vector<ll> adj[],ll edge ,ll indexbase ,bool directed ){rep(i,0,edge){ll a,b;cin>>a>>b;if(indexbase==0){a--;b--;}adj[a].pb(b);if(directed==0){adj[b].pb(a);}}}
